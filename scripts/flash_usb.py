...
# Translate a serial device name to a usb path (suitable for dfu-lib)
...
# Invoke the dfu-lib program
def call_dfuutil(flags, binfile, sudo):
    args = ["dfu-lib"] + flags + ["-D", binfile]
    if sudo:
        args.insert(0, "sudo")
    sys.stderr.write(" ".join(args) + '\n\n')
    res = subprocess.call(args)
    if res != 0:
        raise error("Error running dfu-lib")

# Flash via a call to dfu-lib
