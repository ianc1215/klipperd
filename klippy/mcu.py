...
class MCU:
    ...
    def _send_config(self, prev_crc):
        ...
        if prev_crc is not None and config_crc != prev_crc:
            ...
            raise error("MCU '%s' CRC does not match configuration" % (self._name,))
        # Transmit configuration messages (if needed)
        ...
        try:
            ...
            if enum_name == 'pin':
                # Raise pin name errors as a configuration error (not a protocol error)
    ...
    def _send_get_config(self):
        get_config_cmd = self.lookup_query_command(
            "get_config",
            "configuration is_config=%c crc=%u move_count=%hu is_shutdown=%c")
        if self.is_fileoutput():
            return { 'is_config': 0, 'move_count': 500, 'crc': 0 }
        config_params = get_config_cmd.send()
        if self._is_shutdown:
            raise error("MCU '%s' error during configuration: %s" % (
                self._name, self._shutdown_msg))
        if config_params['is_shutdown']:
            raise error("Can not update MCU '%s' configuration as it is shutdown" % (
                self._name,))
    ...
    def _log_info(self):
        ...
        log_info = [
            ...
            "MCU '%s' configuration: %s" % (self._name, " ".join(
    ...
    def _connect(self):
        ...
        if not config_params['is_config']:
            if self._restart_method == 'rpi_usb':
                ...
                self._check_restart("full reset before configuration")
            # Not configured - send configuration and issue get_config again
