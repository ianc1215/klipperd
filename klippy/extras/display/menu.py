...
class MenuElement(object):
    def __init__(self, manager, config, **kwargs):
        ...
        if config is not None:
            # overwrite class attributes from configuration
    ...
    def _load_script(self, config, name, option=None):
        """Load script template from configuration or callback from dict"""
...
class MenuManager:
    def __init__(self, config, display):
        ...
        # Load local configuration file in same directory as current module
        self.load_config(os.path.dirname(__file__), 'menu.cfg')
        # Load items from main configuration
    ...
    def load_config(self, *args):
        ...
        try:
            ...
            raise self.printer.config_error(
                "Cannot load configuration '%s'" % (filename,))
        if cfg:
