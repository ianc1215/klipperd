...
# Storage of [display_template my_template] configuration sections
...
class PrinterLCD:
    ...
    def load_config(self, config):
        # Load default display configuration file
        pconfig = self.printer.lookup_object('configfile')
        filename = os.path.join(os.path.dirname(__file__), 'display.cfg')
        try:
            dconfig = pconfig.read_config(filename)
        except Exception:
            raise self.printer.config_error("Cannot load configuration '%s'"
                                            % (filename,))
