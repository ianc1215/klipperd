...
def dump_mcu_build():
    ...
    # Try to log last mcu configuration
    dump_file_stats(build_dir, '.configuration')
    try:
        f = open(os.path.join(build_dir, '.configuration'), 'rb')
        data = f.read(32*1024)
        f.close()
        logging.info("========= Last MCU build configuration =========\n%s"
                     "=======================", data)
    except:
        pass
    ...
    try:
        ...
        cparts = ["%s=%s" % (k, v) for k, v in data.get('configuration', {}).items()]
        logging.info("Last MCU build configuration: %s", " ".join(cparts))
    except:
        pass
