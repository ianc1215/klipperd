# Code for reading and writing the Klipper configuration file
#
# Copyright (C) 2016-2021  Kevin O'Connor <kevin@koconnor.net>
#
# This file may be distributed under the terms of the GNU GPLv3 license.
from __future__ import absolute_import
import os, glob, re, time, logging, six.moves.configparser as configparser, io
...
class PrinterConfig:
    ...
    def _read_config_file(self, filename):
        try:
            ...
            msg = "Unable to open configuration file %s" % (filename,)
    ...
    def _find_autosave_data(self, data):
        ...
        if "\n#*# " in regular_data:
            logging.warning("Can't read autosave from configuration file"
    ...
    def _strip_duplicates(self, data, config):
        ...
        # Comment out fields in 'data' that are defined in 'configuration'
        lines = data.split('\n')
        section = None
        is_dup_field = False
    ...
    def _parse_config(self, data, filename, fileconfig, visited):
        ...
        if path in visited:
            raise error("Recursive include of configuration file '%s'" % (filename))
        visited.add(path)
        lines = data.split('\n')
    ...
    def check_unused_options(self, config):
        ...
        # Validate that there are no undefined parameters in the configuration file
        valid_sections = {s: 1 for s, o in access_tracking}
        for section_name in fileconfig.sections():
            section = section_name.lower()
            if section not in valid_sections and section not in objects:
                raise error("Section '%s' is not a valid configuration section"
                            % (section,))
    ...
    def log_config(self, config):
        lines = ["===== Config file =====",
                 ...
                 "======================="]
        self.printer.set_rollover_info("configuration", "\n".join(lines))
    ...
    def get_status(self, eventtime):
        return {'configuration': self.status_raw_config,
    ...
    cmd_SAVE_CONFIG_help = "Overwrite configuration file and restart"
    ...
    def cmd_SAVE_CONFIG(self, gcmd):
        ...
        lines.append("")
        autosave_data = '\n'.join(lines)
        # Read in and validate current configuration file
        ...
        try:
            ...
            msg = "Unable to parse existing configuration on SAVE_CONFIG"
            logging.exception(msg)
            raise gcode.error(msg)
        regular_data = self._strip_duplicates(regular_data, self.autosave)
        self._disallow_include_conflicts(regular_data, cfgname, gcode)

        # IC - Read the old configuration first before we modify it so we have a clean backup to work from.
        old_config = regular_data.rstrip()
        new_config = old_config + autosave_data
        # Determine filenames
        # datestr = time.strftime("-%Y%m%d_%H%M%S")
        # backup_name = cfgname + datestr

        # IC - Look for the configuration file in the klipper configuration directory with the extension ".cfg"
        # Create the backup file name by appending the current date-time stamp to the file before the extension
        if cfgname.endswith(".cfg"):
            backup_name = cfgname[:-4] + time.strftime("-%Y%m%d_%H%M%S") + ".cfg"

        # IC - Specify a directory where we can save our configuration backups, this keeps the configuration root clean
        backup_dir = "/etc/klipperd/backup"  # Default path

        # IC - Since we could run into an issue where the relative path to the backup directory leads to another
        # we should get the absolute path to the configuration directory to be safe.
        backup_path = os.path.abspath(os.path.join(backup_dir, backup_name))

        logging.info("SAVE_CONFIG to '%s' (backup in '%s')",
                     cfgname, backup_path)
        try:
            # IC - Backup the old configuration prior to writing the new configuration
            f = open(backup_path, 'w')
            f.write(old_config)
            f.close(backup_path)

            # IC - Write the new configuration to the configuration directory
            f.open(cfgname)
            f.write(new_config)
            f.close(cfgname)
            ...
            msg = "Unable to write configuration file during SAVE_CONFIG"
            logging.exception(msg)
            raise gcode.error(msg)
        # IC - Disabled automatic restart after saving the configuration file
        # this should at the user's discretion rather than being implied.
        # Request a restart
